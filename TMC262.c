...
/***************************************************************//**
  ...
  This function uses the TMC4361 cover datagram mechanism for
  communicating with the TMC262 motor driver.
********************************************************************/
static void ReadWrite262(UCHAR Which262, UINT *ReadInt, UINT WriteInt)
{
  WriteTMC43xxInt(Which262, TMC43xx_EVENT_CLR_CONF, ~TMC43xx_EV_COVER_DONE);
  ReadTMC43xxInt(Which262, TMC43xx_EVENTS);
  WriteTMC43xxInt(Which262, TMC43xx_COVER_LOW, WriteInt);
  while(!(ReadTMC43xxInt(Which262, TMC43xx_EVENTS) & TMC43xx_EV_COVER_DONE));
  *ReadInt=ReadTMC43xxInt(Which262, TMC43xx_COVER_DRV_LOW);
}


/***************************************************************//**
   \fn Read262StateNoCoverData()
   \brief Get TMC262 status from TMC4361 polling mechanism
   \param Which262  Index of TMC26x to be used (with stepRocker always 0)
   \param Phases  Pointer at UCHAR for phase bits
   \param MStep   Pointer at USHORT for microstep position
   \param StallGuard   Pointer at UINT for stallGuard value
   \param SmartEnergy  Pointer at UCHAR for smartEnergy value
   \param Flags   Pointer at UCAHR for driver error flags

   This function reads status information from the TMC262 motor driver
...
void Read262StateNoCoverData(UCHAR Which262, UCHAR *Phases, USHORT *MStep, UINT *StallGuard, UCHAR *SmartEnergy, UCHAR *Flags)
{
  ...
  SPIReadData=ReadTMC43xxInt(Which262, TMC43xx_POLLING_1_R);

  //Decode the value depending of the choosen TMC262 read back data type.
  switch(DriverConfig[Which262].ReadBackSelect)
  {
    case TMC262_RB_MSTEP:
      if(Phases!=NULL) *Phases=SPIReadData >> 18;
      if(MStep!=NULL) *MStep=(SPIReadData >> 10) & 0x3ff;
      break;

    case TMC262_RB_STALL_GUARD:
      if(StallGuard!=NULL) *StallGuard=SPIReadData >> 10;
      break;
  ...
  if(Flags!=NULL) *Flags=SPIReadData & 0xff;
}
...
void InitMotorDrivers(void)
{
  ...
  for(i=0; i<N_O_MOTORS; i++)
  {
    ...
    DriverConfig[i].StepDirectionDisable=1;
