...
static void InitClosedLoop(UCHAR Axis)
{
  ...
  switch(ClosedLoopInitState[Axis])
  {
    case CS_OPENLOOP:
      ...
      {
        Set262StallGuardCurrentScale(Axis, MotorConfig[Axis].IRun/8);  //Maximalstrom = SAP 6
    ...
    case CS_CLOSEDLOOP:
      ...
      if(!ClosedLoopConfig[Axis].ClosedLoopMode)
      {
        ...
        Set262StallGuardCurrentScale(Axis, 31);  //Current scaling only happens in the TMC4361
...
void SystemControl(void)
{
  ...
  if(abs(GetSysTimer()-Delay)>10)
  {
    ...
    Read262StateNoCoverData(WHICH_262(ActualAxis), NULL, NULL, &StallLevel[ActualAxis], &SmartEnergy[ActualAxis], &DriverFlags[ActualAxis]);
